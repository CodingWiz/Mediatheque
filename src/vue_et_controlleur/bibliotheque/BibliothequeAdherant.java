...
public class BibliothequeAdherant extends Stage {
	...
	public BibliothequeAdherant() {
		try {
			...
			this.setScene(scene);
			this.setTitle("Bibliotheque Adhérent");
			this.sizeToScene();
			this.show();
		} catch(Exception e) {
			e.printStackTrace();
	...
	private VBox createTableDocumentVBox() {
	    ...
	    for (int i = 0; i < ListeDVD.getLstDVDATrouver().size(); i++){
	    	lstAllDocument.add(new Document(ListeDVD.getLstDVDATrouver().get(i).getNoDoc(),ListeDVD.getLstDVDATrouver().get(i).getTitre(),ListeDVD.getLstDVDATrouver().get(i).getDateParution(),ListeDVD.getLstDVDATrouver().get(i).getEtat(),ListeDVD.getLstDVDATrouver().get(i).getMotsclés()));
	    }
	    for (int i = 0; i < ListeLivre.getLstLivreATrouver().size(); i++){
	    	lstAllDocument.add(new Document(ListeLivre.getLstLivreATrouver().get(i).getNoDoc(),ListeLivre.getLstLivreATrouver().get(i).getTitre(),ListeLivre.getLstLivreATrouver().get(i).getDateParution(),ListeLivre.getLstLivreATrouver().get(i).getEtat(),ListeLivre.getLstLivreATrouver().get(i).getMotsclés()));
	    }
	    for (int i = 0; i < ListePeriodique.getLstPeriodiqueATrouver().size(); i++){
	    	lstAllDocument.add(new Document(ListePeriodique.getLstPeriodiqueATrouver().get(i).getNoDoc(),ListePeriodique.getLstPeriodiqueATrouver().get(i).getTitre(),ListePeriodique.getLstPeriodiqueATrouver().get(i).getDateParution(),ListePeriodique.getLstPeriodiqueATrouver().get(i).getEtat(),ListePeriodique.getLstPeriodiqueATrouver().get(i).getMotsclés()));
	    }
