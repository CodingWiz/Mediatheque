package vue_et_controlleur.bibliotheque;
  ...
  //  ArrayList<Livre> lstAllLivre = LectureDesFichiers.LireFichierLivres("Livres.txt");
	...
	public BibliothequePreposer  () {
		try {
			...
			this.setScene(scene);
			this.setTitle("Bibliotheque Préposé");
			this.sizeToScene();
			this.show();
		} catch(Exception e) {
			e.printStackTrace();
	...
	private VBox createVboxFiltres() {
		...
		btnSupprimerDocument.setOnAction(e ->{
			...
			if(listView.getSelectionModel().getSelectedItem() == null){
				...
				alertAttention.setContentText("Vous devez sélectionner le document de la liste que vous aimeriez supprimer");
	...
	private VBox createTableDocumentVBox() {
	   ...
	   /* for (int i = 0; i < ListeDVD.getLstDVDATrouver().size(); i++){
	    	ListDocument.getLstAllDocument().add(new Document(ListeDVD.getLstDVDATrouver().get(i).getNoDoc(),ListeDVD.getLstDVDATrouver().get(i).getTitre(),ListeDVD.getLstDVDATrouver().get(i).getDateParution(),ListeDVD.getLstDVDATrouver().get(i).getEtat(),ListeDVD.getLstDVDATrouver().get(i).getMotsclés()));
	    }
	    for (int i = 0; i < ListeLivre.getLstLivreATrouver().size(); i++){
	    	ListDocument.getLstAllDocument().add(new Document(ListeLivre.getLstLivreATrouver().get(i).getNoDoc(),ListeLivre.getLstLivreATrouver().get(i).getTitre(),ListeLivre.getLstLivreATrouver().get(i).getDateParution(),ListeLivre.getLstLivreATrouver().get(i).getEtat(),ListeLivre.getLstLivreATrouver().get(i).getMotsclés()));
	    }
	    ...
	    TableColumn<Document, Integer> colonneNoDoc = new TableColumn<Document, Integer> ("Numéro du document");
	...
	private VBox createTablePeriodiqueVBox() {
	    ...
	    TableColumn<Periodique, Integer> colonneNoPeriodique = new TableColumn<Periodique, Integer> ("Numéro du periodique");
